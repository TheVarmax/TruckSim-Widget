...
* [TrucksBook Client](https://trucksbook.eu/), встановлений і з виконаним входом до акаунта (потрібен для відстеження кілометражу)

### 1. Завантажте TruckSim Widget

1. Відкрийте [останній реліз](https://github.com/TheVarmax/TruckSim-Widget/releases/latest).
2. У розділі **Assets** завантажте `.zip`-архів TruckSim Widget.
3. Після завершення завантаження натисніть на архів правою кнопкою миші та виберіть **Extract All... / Видобути все...**.
4. Виберіть будь-яку папку, де зберігатиметься віджет, наприклад `Документи\TruckSim Widget`.

> Не запускайте застосунок прямо з ZIP-архіву. Спочатку повністю розпакуйте весь архів.

### 2. Встановіть плагін телеметрії

У розпакованій папці TruckSim Widget відкрийте папку `plugin`. У ній знаходиться файл `scs-telemetry.dll`.

Скопіюйте `scs-telemetry.dll` у папку `plugins` кожної гри, з якою хочете використовувати TruckSim Widget.
...
> У папку `plugins` гри потрібно скопіювати лише `scs-telemetry.dll`. Решту файлів TruckSim Widget залиште разом у папці, куди було розпаковано архів.

### 3. Запустіть віджет

1. Запустіть TrucksBook Client і переконайтеся, що ви увійшли до акаунта.
2. Запустіть ETS2 або ATS і завантажте свій профіль.
3. Відкрийте `TruckSim Widget.exe` з розпакованої папки TruckSim Widget.
4. Перевірте, що віджет показує статус онлайн для TrucksBook і підключену телеметрію.
5. Можна їхати.
